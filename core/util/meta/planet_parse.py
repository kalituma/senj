...
def parse_planet(metafile):
    ...
    if ext == '.json': ## json metadata
        ...
        for t in root_node.xpath('//eop:Platform', namespaces=namespaces):
        ...
        for t in root_node.xpath(main_tag, namespaces=namespaces):
            band = {}
            for i,tag in enumerate(tags):
                    nodes = t.xpath(tag, namespaces=namespaces)
                    if len(nodes) > 0:
        ...
        for t in root_node.xpath(f'//{main_tag}', namespaces=namespaces):
            for i,tag in enumerate(tags):
                    nodes = t.xpath(tag)
                    if len(nodes) > 0:
        ...
        for t in xmldoc.getElementsByTagName(main_tag):
            for i,tag in enumerate(tags):
                    nodes = t.getElementsByTagName(tag)
                    if len(nodes) > 0:
        ...
        for t in xmldoc.getElementsByTagName(main_tag):
            for i,tag in enumerate(tags):
                    ...
                    for j,tag2 in enumerate(['{}:latitude'.format(metadata['satellite_prefix']),'{}:longitude'.format(metadata['satellite_prefix'])]):
                        node2 = nodes[0].getElementsByTagName(tag2)
                        if len(node2) > 0:
