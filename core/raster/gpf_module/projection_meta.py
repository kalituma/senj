...
def _make_dct(grids, s2_target_res, p, proj4_string, is_utm, is_ps, zone_name):
...
def projection_from_meta(meta, s2_target_res=10, return_grids=False):
    ...
    or_grids = _build_grid(meta, 'OR_GRIDS')
    cur_grids = _build_grid(meta, 'CUR_GRIDS')

    ## select target grid here and return projection dict
    or_dct = _make_dct(or_grids, s2_target_res, p, proj4_string, is_utm, is_ps, zone_name)
    cur_dct = _make_dct(cur_grids, s2_target_res, p, proj4_string, is_utm, is_ps, zone_name)

    if return_grids:
        return(or_dct, cur_dct, or_grids, cur_grids)
    else:
        return(or_dct, cur_dct)