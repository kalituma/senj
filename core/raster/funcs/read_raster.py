...
def load_raster(raster:Raster, in_module:RasterType, selected_bands:list[Union[str, int]]=None, bname_word_included:bool=False):
    ...
    if raster.module_type == RasterType.GDAL:
        ...
        meta_dict, raw, selected_bands = load_raster_gdal(path, selected_bands=selected_bands)
        ...
        meta_dict, raw, selected_bands = load_raster_gpf(path, selected_bands, bname_word_included)
    else:
...
def read_band_from_raw(raster:Raster, selected_band, module_type:RasterType) -> Raster:
