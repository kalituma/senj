from core.util import assert_bnames, remove_list_elements, deprecated
...
def update_raw_from_cache(raster:Raster, clear=False):
    ...
    if raster.is_band_cached:
        ...
        if module_type == ModuleType.SNAP:
            ...
            if raster.get_band_names() != raster.get_band_names('raw'):
            ...
            if not clear:
                if len(not_cached_bands) > 0:
                    ...
                    raster.reorder_bands(cached_order, sort=False)
            else:
                raster.reorder_bands(cached_bands, sort=False)

            raster.to_have_same_dtype()
