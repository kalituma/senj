...
def polylakes(database = 'worldlakes', remove_zip = False):
    ...
    if database.lower() == 'worldlakes':
        ...
        local_zip = '{}/{}'.format(atmos.config['external_dir'], 'World_Lakes-shp.zip')
        local_file = '{}/{}'.format(atmos.config['external_dir'], 'World_Lakes-shp/World_Lakes.shp')
        local_dir = '{}/{}'.format(atmos.config['external_dir'], 'World_Lakes-shp')
        ...
        local_zip = '{}/{}'.format(atmos.config['external_dir'], 'HydroLAKES_polys_v10_shp.zip')
        local_file = '{}/{}'.format(atmos.config['external_dir'], 'HydroLAKES_polys_v10_shp/HydroLAKES_polys_v10_shp/HydroLAKES_polys_v10.shp')
        local_dir = '{}/{}'.format(atmos.config['external_dir'], 'HydroLAKES_polys_v10_shp')
    elif database.lower() == 'gshhg':
        url = 'https://www.ngdc.noaa.gov/mgg/shorelines/data/gshhg/latest/gshhg-shp-2.3.7.zip'
        local_zip = '{}/{}'.format(atmos.config['external_dir'], 'gshhg-shp-2.3.7.zip')
        local_file = '{}/{}'.format(atmos.config['external_dir'], 'gshhg-shp-2.3.7/GSHHS_shp/f/GSHHS_f_L1.shp')
        local_dir = '{}/{}'.format(atmos.config['external_dir'], 'gshhg-shp-2.3.7')
    ...
    if not os.path.exists(local_file):
        ...
        if not os.path.exists(local_zip):
            ...
            ret = atmos.shared.download_file(url, local_zip)
