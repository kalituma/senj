...
def copernicus_dem_download(lon, lat, source = 'copernicus30'):
    local_dir = atmos.config['copernicus_dem_dir']
    ...
    if (not os.path.exists(tilelist)):
        if (server == 'amazon'):
            ...
            atmos.shared.download_file(download_url, tilelist)
        if (server == 'copernicus'):
            print('Getting Copernicus DEM file lists from PRISM. This can take a few minutes.')
            atmos.dem.copernicus_dem_filelist()

    ## read tiles
    tiles = atmos.setting.read_list(tilelist)
    ...
    if tile in tiles:
        if server == 'amazon':
            ...
            if not os.path.exists(local_file):
                atmos.shared.download_file(download_url, local_file)
        if server == 'copernicus':
            ...
            if not os.path.exists(local_file):
                ...
                if not os.path.exists(local_tar): atmos.shared.download_file(download_url, local_tar)
