...
def hgt_lonlat(lon1, lat1, nearest=True, hgt_dir=None, source = 'srtmgl3'):
    ...
    hgt_files, hgt_required = atmos.dem.hgt_find(limit, required=True, hgt_dir=hgt_dir, hgt_url=url_base)
    ...
    for i, hgt_file in enumerate(hgt_files):
        ...
        hgt = atmos.dem.hgt_read(hgt_file)

        if (type(lon1) is float) & (type(lat1) is float):
            lon0,lat0 = atmos.dem.hgt_geolocation(hgt_file, grid=False)
            hgtip = interpolate.RectBivariateSpline(lon0,lat0, hgt)
            result = hgtip(lon1,lat1)
        else:
            lon0,lat0 = atmos.dem.hgt_geolocation(hgt_file, grid=True)
            ## reproject
            result = atmos.shared.reproject2(hgt, lon0, lat0, lon1, lat1, nearest=nearest)
