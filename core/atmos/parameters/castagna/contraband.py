...
def contraband(gem, verbosity=5):
    ...
    if type(gem) is str:
        gem = atmos.gem.gem(gem)
        nc_projection = gem.nc_projection
    gemf = gem.file

    ## compute contrabands
    cb_file = atmos.config['data_dir']+'/Shared/algorithms/Castagna/{}_contra_coefficients.txt'.format(gem.gatts['sensor'])
    if os.path.exists(cb_file):
        ## load config for this sensor
        cb_cfg = atmos.shared.import_config(cb_file)
        ...
        ## make bands dataset
        rsrd = atmos.shared.rsr_dict(gem.gatts['sensor'])[gem.gatts['sensor']]
        gem.bands = {}
        ...
        if compute_contraband:
            ...
            rsrd_cb = atmos.shared.rsr_dict(sensor_cb)[sensor_cb]
