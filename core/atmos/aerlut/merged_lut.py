...
from core.atmos.nc import lutnc_write
...
def merged_lut(lut, lutint, pressure, sensor = None, get_remote = True, lut_par = None, store = True, override = False):
    ...
    lutdir = '{}/{}'.format(atmos.config['lut_dir'], '-'.join(lutid.split('-')[0:3]))
    ...
    if (not os.path.exists(mlut_nc)) | (override):
        ...
        lut_data, lut_meta = atmos.aerlut.import_lut(lutid, lutdir, sensor = sensor, lut_par = None, get_remote = get_remote)
        rskyd = atmos.aerlut.import_rsky_luts(sensor = sensor, models = [model], lutbase = lutint, get_remote = get_remote)
        ...
        if store:
            ...
            lutnc_write(mlut_nc, lut_data, lut_meta, dims = dim_names, compression = False)
            #atmos.shared.lutnc_write(mlut_nc, lut_data, lut_meta, dims = dim_names, compression = True, complevel = 1)

    else:
        print('Reading {}'.format(os.path.basename(mlut_nc)))
        lut_data, lut_meta = atmos.shared.lutnc_import(mlut_nc)
