...
def irradiance(aot, sza, mod = 2, date = None, apply_cosine = True, settings = {}):
    ...
    ## parse settings
    setu = atmos.setting.parse(None, settings=settings, merge=True)
    ...
    tg_hyp = atmos.ac.gaslut_interp(sza, vza, pressure=pressure, lutconfig='202402F',
                            pars = ['dtdica','dtoxyg','dtniox','dtmeth'])

    ## compute ozone transmittance
    tg_o3 = atmos.ac.tto3_interp(sza, vza, uoz=uoz, total=False)

    ## interpolate to same grid
    tg_hyp['dto3'] = np.interp(tg_hyp['wave'], tg_o3['wave'], tg_o3['dt_o3'])

    ## compute water transmittance
    wv_wv_hs, dt_wv_hs = atmos.ac.wvlut_interp(sza, vza, uwv=uwv, pressure=pressure, par = 'dtwava', config = '202402CP')
    ...
    lutdw = atmos.aerlut.import_luts(add_rsky=False, par='romix', sensor=None, base_luts=[lutid])
