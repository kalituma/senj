...
def gas_transmittance(sza, vza, pressure = 1013, waves = None, uoz = 0.3, uwv = 1.5,
    ...
    if sensor is None:
        ...
        wv_wv_hs, tt_wv_hs = atmos.ac.wvlut_interp(sza, vza, uwv=uwv)
        ...
        for g in gases:
            d['tt_gas'] *= d['tt_{}'.format(g)]

        ## resample to rsr
        if rsr is not None:
            for k in d:
                d[k] = atmos.shared.rsr_convolute_dict(waves, d[k], rsr)
    else:
        ## compute co2, o2, n2o, ch4 transmittance
        tg_ret = atmos.ac.gaslut_interp(sza, vza, pressure=pressure, lutconfig=lutconfig, sensor=sensor)

        ## compute water transmittance
        wv_ret = atmos.ac.wvlut_interp(sza, vza, uwv=uwv, sensor=sensor)

        ## compute ozone transmittance
        tt_o3 = atmos.ac.tto3_interp(sza, vza, uoz=uoz, sensor=sensor)
