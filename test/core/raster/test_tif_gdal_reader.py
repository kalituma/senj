...
from core.raster.funcs.reader import TifGdalReader, SafeGdalReader, SafeSnapReader
...
class TestGdalReader(unittest.TestCase):
    ...
    def test_read_safe_snap(self):
        ...
        with self.subTest('safe s1'):
            ...
            reader = SafeSnapReader(s1_grdh_path)
            raster = reader.read()
            ...
            reader = SafeSnapReader(s1_slc_path)
            raster = reader.read()
            print(raster.meta_dict)
    ...
    def test_read_dim_snap(self):
        ...
        with self.subTest('dim s1'):
            ...
            reader = SafeSnapReader(s1_dim_path)
            raster = reader.read()
            print(raster.meta_dict)
        ...
        with self.subTest('dim s2'):
            ...
            reader = SafeSnapReader(s2_dim_path)
            raster = reader.read()
            print(raster.meta_dict)
